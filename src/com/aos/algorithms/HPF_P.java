...
public class HPF_P extends Algorithm 
{    
    @SuppressWarnings({ "unchecked", "rawtypes" })
	@Override
    public Queue<Process> schedule(PriorityQueue<Process> q) 
    {
        int end_time = 0, start_time;
        Process p, p_scheduled;
        Process left_out;
        ...
        while (!q.isEmpty() || !ready_processes.isEmpty() || !waiting_processes.isEmpty())
        {
            ...
            if (ready_processes.isEmpty())
                ...
                p = (ready_processes.peek().getPriority() <= waiting_processes.peek().getPriority()) ? ready_processes.poll() : waiting_processes.poll();
            ...
            if (p.getBurstTime() > 1)
            {
                ...
                {
                    Logger.getLogger(HPF_P.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
            else 
            {
